...
/**
 * The body every endpoint answers with, one shape whether the call succeeded or failed.
 *
 * @param code the HTTP status of the verdict
 * @param message a description for the caller, disclosing nothing they may not see
 * @param data the payload, absent on failure
 * @param total records across all pages, absent when the payload is not a page
...
public record HttpResponse<T>(int code, String message, T data, Long total) {
	...
	/**
	 * The only status a success is answered with.
	 */
