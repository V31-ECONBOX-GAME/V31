...
/**
 * Answers anything that reached the transport unrecognised. Asked last; without it the
 * framework falls back to {@code UNKNOWN} with nothing attached.
 * <p>
 * The message is not passed through. A driver or serializer message quotes a fragment of
 * the request, a column name, a class that failed — disclosing how the service is built
 * to anybody able to provoke an error. The cause is logged here instead.
 * <p>
 * A {@link StatusRuntimeException} raised deliberately is left alone: already phrased for
 * the wire.
 *
 * @author Xander Wang
 * @since 0.2.0
