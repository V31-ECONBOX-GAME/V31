...
/**
 * Reports a write the database refused as the caller's conflict ({@code 409}) rather than
 * the server's fault ({@code 500}), which would tell the caller to retry a request that
 * will never succeed.
 * <p>
 * Ordered ahead of {@link HttpResponseExceptionHandler} so its {@link Exception} handler
 * does not claim these first. Nothing of the failure is disclosed: the message names the
 * constraint, the table and often the conflicting value.
 *
 * @author Xander Wang
 * @since 0.2.0
 */
...
public class DataAccessExceptionHandler {
	...
	/**
	 * Answer a write that lost a race for the same row. Unlike the above, retrying after
	 * a re-read is the correct response rather than a futile one.
